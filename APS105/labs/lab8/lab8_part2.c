...
int minimax(char board[26][26], int n, char turn, int depth, int alpha, int beta, bool maximizingPlayer) {
    ...
    if (depth == 0) {
        return currentScore(board, n, turn);
    }
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                if (board[i][j] == 'U') {
                    ...
                    if (currentScore > 0) {
                        board[i][j] = turn;
                        int value = minimax(board, n, turn, depth - 1, alpha, beta, false);
                        board[i][j] = 'U';
            ...
            for (int j = 0; j < n; ++j){
                if (board[i][j] == 'U') {
                    ...
                    for (int k = 0; k < 8; k++) { // Check for all directions
                        ...
                        while (positionInBounds(n, r, c) && board[r][c] != 'U' && board[r][c] != opponent) {
                            flips++;
                            r += directions[k][0];
                            c += directions[k][1];
                    ...
                    if (currentScore > 0) {
                        board[i][j] = opponent;
                        int value = minimax(board, n, turn, depth - 1, alpha, beta, true);
                        board[i][j] = 'U';
...
int makeMove(char board[26][26], int n, char turn, int *row, int *col) {
    ...
    int depth = 5; // You can adjust the depth according to the desired difficulty
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (board[i][j] == 'U') { // Check for empty space on the board
                ...
                if (currentScore > 0) {
                    board[i][j] = turn;
                    int value = minimax(board, n, turn, depth - 1, INT_MIN, INT_MAX, false);
                    board[i][j] = 'U';
